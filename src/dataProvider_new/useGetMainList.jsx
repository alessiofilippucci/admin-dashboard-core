 ...
 * };
 */
export const useGetMainList = (resource, pagination, sort, filter, options) => {
    const requestSignature = JSON.stringify({ pagination, sort, filter });
    const memo = useRef({});
    const { data: { finalIds, finalTotal, allRecords }, error, loading, loaded, } = useQueryWithStore({ type: 'getList', resource, payload: { pagination, sort, filter } }, options,
        ...
        (state) => {
            ...
            const allRecords = get(state.admin.resources, [resource, 'data'], defaultData);
    ...
    const data = useMemo(() => typeof finalIds === 'undefined'
        ? defaultData
        : finalIds
