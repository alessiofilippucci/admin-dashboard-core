...
import { linkToRecord, sanitizeListRestProps, useListContext, Utils, } from 'admin-dashboard-core';
        ...
        >
            {ids.map((id, index) => {
                ...
                if (resourceLinkPath) {
                    return (
                        <Link
                            className={classnames(classes.link, fullWidth ? 'w-100' : '', lineClassName)}
                            key={id}
                            to={resourceLinkPath}
                ...
                return (
                    <div
                        className={classnames(fullWidth ? 'w-100' : '', lineClassName)}
                        key={id}
                    >
                        {
                            props.addLabel || children.props.addLabel ? (
                                <Labeled
                                    label={props.label || (Utils.IsFunction(children.props.label) ? children.props.label(data[id]) : children.props.label)}
                                    source={props.source || children.props.source}
                                    basePath={basePath}
                                    record={data[id]}
                                    resource={resource}
                                    asStandardField={asStandardField}
                                    className={useBootstrap && fullWidth ? 'w-100' : ''}
