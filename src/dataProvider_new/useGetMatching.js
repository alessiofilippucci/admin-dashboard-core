...
const useGetMatching = (resource, pagination, sort, filter, source, referencingResource, options) => {
    ...
    const { data: possibleValues, total, error, loading, loaded, } = useQueryWithStore({
        type: 'getList',
        resource,
        payload,
    }, Object.assign(Object.assign({}, options), { relatedTo, action: CRUD_GET_MATCHING }), (state) => getPossibleReferenceValues(state, {
        referenceSource,
        resource: referencingResource,
        source,
    }), (state) => get(state.admin.resources, [
        resource,
        'list',
        'cachedRequests',
    ...
    const referenceState = useSelector(state => getReferenceResource(state, {
        reference: resource,
    }));
