...
const useGetMany = (resource, ids, options = {}) => {
    ...
    if (!isEqual(state.data, data)) {
        setState(Object.assign(Object.assign({}, state), { data, loaded: true }));
    }
 ...
 */
const makeGetManySelector = () => createSelector([
    (state, resource) => get(state, ['admin', 'resources', resource, 'data']),
    (_, __, ids) => ids,
    ...
    resources.forEach(resource => {
        ...
        dataProvider
            ...
            .then(response =>
                ...
                ReactDOM.unstable_batchedUpdates(() => queries.forEach(({ ids, setState, onSuccess }) => {
            ...
            .catch(error => ReactDOM.unstable_batchedUpdates(() => queries.forEach(({ setState, onFailure }) => {
